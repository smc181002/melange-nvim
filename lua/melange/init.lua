...
---@alias GitSigns "default" | "github" | "none" | ""
...
Melange.config = vim.deepcopy(defaultConfig)
...
-- package.loaded['melange/init.lua']
...
local github_gitsigns = palette.github_gitsigns -- Foreground colors
...
local function get_gitsigns_group()
  local defaultGitSignsGroup = {
    GitSignsAdd = { fg = c.green },
    GitSignsChange = { fg = c.magenta },
    GitSignsDelete = { fg = c.red },
  }

  local githubGitSignsGroup = {
    GitSignsAdd = { fg = github_gitsigns.green },
    GitSignsChange = { fg = github_gitsigns.yellow },
    GitSignsDelete = { fg = github_gitsigns.red },
  }

  local gitSignsTable = {}

  if Melange.config.gitsigns == "default" then
    gitSignsTable = defaultGitSignsGroup
  elseif Melange.config.gitsigns == "github" then
    gitSignsTable = githubGitSignsGroup
  end

  return gitSignsTable
end

--- main load function
Melange.load = function()
  ...
  local gitSignsGroup = get_gitsigns_group()

  local merged_groups = merge(groups, gitSignsGroup)

  -- add highlights
  for name, attrs in pairs(merged_groups) do
  ...
  -- for name, attrs in pairs(gitSignsTable) do
  --   vim.api.nvim_set_hl(0, name, attrs)
  -- end
...
---@param opts? Melangeconfig
function Melange.setup(opts)
  Melange.config = vim.tbl_deep_extend("force", Melange.config, opts or {})
end
